...
interface ReceptionRegisterPageProps {
  onLogin: (user: User) => void;
}

const ReceptionRegisterPage: React.FC<ReceptionRegisterPageProps> = ({ onLogin }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Cadastro da recepção realizado com sucesso! Você já pode acessar sua conta.');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0fdf4] relative overflow-hidden">
      ...
      <div className="flex-grow flex items-center justify-center p-4 relative z-10 py-12">
        <div className="bg-white/95 backdrop-blur-2xl rounded-[40px] shadow-[0_32px_64px_-16px_rgba(21,128,61,0.15)] w-full max-w-4xl p-8 md:p-12 flex flex-col md:flex-row gap-12 border border-white/50 modern-shadow">
          {/* Lado Esquerdo - Info */}
          <div className="md:w-1/3 flex flex-col">
            <div className="flex justify-between items-start mb-10">
              <button
                onClick={() => navigate('/reception/login')}
                className="text-slate-400 hover:text-[#15803d] transition-all bg-slate-50 w-10 h-10 rounded-xl flex items-center justify-center border border-slate-100 shadow-sm"
              >
            ...
            <div className="space-y-4 mb-10">
              <h1 className="text-4xl font-extrabold text-[#15803d] tracking-tight leading-tight">Nova Recepção</h1>
              <p className="text-sm text-slate-500 font-medium leading-relaxed">
                Habilite seu acesso ao Portal de Atendimento para gerenciar cadastros e recepção de pacientes no Laboratório Municipal.
              </p>
            </div>

            <div className="mt-auto bg-emerald-50 p-8 rounded-[32px] border border-emerald-100 relative overflow-hidden">
              <div className="flex items-center gap-3 text-emerald-600 mb-3">
                <i className="fas fa-id-badge text-lg"></i>
                <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Atendimento</span>
              </div>
              <p className="text-[12px] text-emerald-900/70 leading-relaxed font-semibold">
                Sua conta será verificada pela administração do laboratório.
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2 space-y-1.5">
                ...
                <div className="relative group">
                  <i className="fas fa-user absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-600 transition-colors"></i>
                  <input
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-14 pr-12 py-4 rounded-2xl border border-slate-100 bg-slate-50/50 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 outline-none text-sm font-bold text-slate-700 transition-all"
                    value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })}
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-emerald-600">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <i className="fas fa-check-double absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-600 transition-colors"></i>
                  <input
                    required type={showConfirmPassword ? "text" : "password"} maxLength={6} placeholder="••••••"
                    className="w-full pl-14 pr-12 py-4 rounded-2xl border border-slate-100 bg-slate-50/50 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 outline-none text-sm font-bold text-slate-700 transition-all"
                    value={formData.confirmPassword} onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })}
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-emerald-600">
              ...
              <button type="submit" className="md:col-span-2 w-full bg-[#15803d] text-white font-extrabold py-5 rounded-3xl shadow-2xl hover:bg-emerald-900 hover:-translate-y-0.5 transition-all mt-6 uppercase tracking-widest text-xs flex items-center justify-center gap-4 active:scale-95">
                Confirmar Registro de Atendimento
