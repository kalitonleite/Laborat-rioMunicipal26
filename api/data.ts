...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    switch (req.method) {
      case 'POST': // Unified endpoint for select, insert, update, delete
        if (action === 'select') {
        ...
        if (action === 'update') {
            ...
            query += ' WHERE ' + filterKeys.map((key, i) => {
        ...
        if (action === 'upsert') {
            ...
            query += ` ON CONFLICT (${conflictKeys.join(', ')}) DO UPDATE SET ` + keys.map((key, i) => `${key} = EXCLUDED.${key}`).join(', ');
            query += ' RETURNING *';
